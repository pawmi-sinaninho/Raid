    ...
    <section className="gigalodon-workspace" data-testid="gigalodon-command-center">
      <div className="gigalodon-layout">
        ...
        <div className="gigalodon-workbench">
          ...
          ].map((warning)=><p key={warning}><StatusStamp tone="source">LIVE_REQUIRED</StatusStamp>{warning}</p>)}</section>
          ...
          {(snapshot.session.status === "FINAL_ACTIVE" || state.final.startedAt) && <div className="panel final-tracker" data-testid="gigalodon-final-tracker">
            ...
            <div className="final-damage-grid"><Metric label="Résultat" value={state.final.result ?? "EN COURS"} /><Metric label="Dégâts totaux" value={state.final.totalDamage.toLocaleString("fr-CH")} /><Metric label="Tours enregistrés" value={state.final.damageRounds.map((entry)=>`T${entry.round}: ${entry.cumulativeDamage.toLocaleString("fr-CH")}`).join(" · ")||"—"} /><Metric label="Score ressources" value={(state.final.confirmedResourceScore??state.confirmedScore).toLocaleString("fr-CH")} /><Metric label="Bonus final" value={(state.final.finalBonusScore??state.final.projectedBonusScore).toLocaleString("fr-CH")} /><Metric label="Score total" value={(state.final.totalScore??state.confirmedScore+state.final.projectedBonusScore).toLocaleString("fr-CH")} /></div>
            <div className="final-controls">
              ...
              <button className="primary" disabled={Boolean(state.final.result)} onClick={() => void safe({ type: "UPDATE_GIGALODON_FINAL", combatRound: round, totalDamage: damage, completed: true, result: "VICTORY" })}>VICTORY</button>
              <button className="danger" disabled={Boolean(state.final.result)} onClick={() => void safe({ type: "UPDATE_GIGALODON_FINAL", combatRound: round, totalDamage: damage, completed: true, result: "DEFEAT" })}>DEFEAT</button>
